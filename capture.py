...
class StarlinkSigMFCapture:
    ...
    def compress_samples(self,
        ...
        f, t, STFT = signal.stft(rawData, fs=sample_rate, nperseg=nfft, noverlap=noverlap, return_onesided=False)
        ...
        rawData = np.append(
        samples,
        np.zeros((nfft - len(samples) % (nfft - noverlap)) % nfft, dtype=complex)
        )
        ...
        for i in range(numFrames):
            ...
            relVels = c*f[largeInds]/center_freq 
        ...
        output_file = np.column_stack((outputFreqs,outputVals))
